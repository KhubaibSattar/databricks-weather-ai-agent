...
_SCOPE = os.environ.get("LAKEBASE_SECRET_SCOPE", "database-hw2")
_KEY = os.environ.get("LAKEBASE_SECRET_KEY", "lakebase-weather-url")
...
def run_insert_returning(sql: str, params: tuple | dict | None = None) -> list[dict]:
    """Run an INSERT/UPDATE with RETURNING clause, commit, and return the rows."""
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(sql, params)
            result = cur.fetchall()
